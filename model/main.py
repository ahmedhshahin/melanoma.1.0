...
train_object = Training(model = FullResulotionNet, model_params = {'in_channels': 3, 'n_outputs': 1, 'n_filters' : 128}, criterion = soft_dice_loss, val_metric = val_metric, initial_lr = 5e-5, dataset = Melanoma, dataset_params = {'data_path': '/content/melanoma.1.0/dataset/2016data/train/', 'test_path' : '/content/melanoma.1.0/dataset/2016data/test/', 'transforms' : 'd'}, batch_size_train = 4, train_steps_before_update = 4, batch_size_val = 4, cuda_device = 0, data_parallel = False, test_mode = False)
