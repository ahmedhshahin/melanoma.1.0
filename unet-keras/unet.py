...
class myUnet(object):
	...
	def get_unet(self):
		...
		def soft_dice(y_pred, y_true):
			...
			y_pred = K.reshape(y_pred, (tf.shape(y_pred)[0]*tf.shape(y_pred)[1]*tf.shape(y_pred)[2],2)) 
			y_true = K.reshape(y_true, (tf.shape(y_true)[0]*tf.shape(y_true)[1]*tf.shape(y_true)[2],2)) 
